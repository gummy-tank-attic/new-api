...
export function parseTiersFromExpr(exprStr: string): ParsedTier[] {
  ...
  if (compiled.status === 'ready') {
    ...
    const timeTiers = readTimeTokenPricing(exprStr)?.tiers.map(mapTokenTier)
    if (timeTiers && timeTiers.length > 0) return timeTiers
  }
  const split = splitTopLevelMultiply(exprStr.trim())
  if (split.length > 1) {
    const base = split[0]
    ...
    if (isTrailingPeakOffPeakScale(rest)) {
      ...
      if (compiledBase.status === 'ready') {
        const canonical = readTokenTierChain(compiledBase.ast)
        if (canonical) return canonical.map(mapTokenTier)
      }
    }
  }
